...
from bam_mol.training.trainer import (energy_forces_fn,
...
def get_energy(data_list, 
    ...
    for graphset in data_list:
        ...
        prd_enr += list(jnp.array([enr+graph_enr_avg]).reshape(-1))
...
def get_exact(data_list, model_ckpt):
    ...
    for graph_data in data_list:
        ...
        exact_enr += list(jnp.array(graph_data.globals['energy'].reshape(-1)[:-1] + graph_enr_avg))
